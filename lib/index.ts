...
let baseURL = "http://localhost:10001"
let proxyKey = ""
 ...
 */
export function connectBridge(key: string, proxyTarget: string, url = baseURL) {
  proxyKey = key
  const request = () =>
    axios({
      baseURL: url,
...
export function cancelBridge() {
  ...
  return axios({ baseURL, url: "/cancel-proxy?key=" + proxyKey, method: "post" })
}
...
export function requestConfigKey(...keys: string[]): Promise<any[]> {
  return axios({ baseURL, url: "/get-config", method: "post", data: keys }).then((res) => {
...
export function setConfig(key: string, value: any) {
  ...
  return axios({ baseURL, url: "/set-config?key=" + key, method: "post", data: value, headers })
}
...
export function deleteConfig(key: string) {
  ...
  // 向服务器发送删除配置项的请求
  return axios({ baseURL, url: "/delete-config?key=" + key, method: "post" })
}
...
function startListen() {
  ...
  const task = new AsyncTask(SCHEDULE_CONFIG, async () => {
    ...
    // 从服务器请求最新的配置数据。
    const res = await axios({ baseURL, url: "/get-config", method: "post", data: keys })
